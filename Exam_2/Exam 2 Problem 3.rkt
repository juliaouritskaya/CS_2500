...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |Exam 2 Problem 3|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Problem 3
;; 
...
;; repeat-right : [NEListof String] -> [NEListof String]
;; Produces a non-empty list of strings where each string is the output
;; of appending all string to the right of the corresponding string.

(check-expect (repeat-right (list "Hi" "Hello" "Goodbye" "Bye"))
              (list "HiHelloGoodbyeBye" "HelloGoodbyeBye" "GoodbyeBye" "Bye"))

(check-expect (repeat-right (list "hi"))
              (list "hi"))
...
(define (repeat-right alist)
  (cond
    ...
    [(cons? (rest alist))
     (cons 
      (string-append (first alist) (repeat (rest alist)))
      (repeat-right (rest alist)))])) 

;; repeat : [NEListof String] -> String
;; Appends a non-empty list of strings into one string.

(check-expect (repeat (list "hi" "hello"))
              "hihello")

(check-expect (repeat (list "bye" "goodbye"))
              "byegoodbye")

(define (repeat nelos)
  (foldr string-append "" nelos))
