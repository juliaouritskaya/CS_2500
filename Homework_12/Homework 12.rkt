...
;; visit : (X) [Listof X] Nat[>0] -> [Listof X]
;; Groups the input list into sub-lists of supplied length;
;; visits each item at most once.

(check-expect (visit '() 2) '())
(check-expect (visit (list 1 2 3) 1) (list (list 1) (list 2) (list 3)))
(check-expect (visit (list 1 2 3) 2) (list (list 1 2) (list 3)))

(define (visit lox nat)
  (cond
    [(empty? lox) '()]
    [(cons? lox)
     (local [(define TD (take-and-drop lox nat))]
       (cons
        (td-take TD)
        (visit (td-drop TD) nat)))]))
    
(define-struct td [take drop])

;; A TakeDrop is a (make-td [Listof X] [Listof X])
;; - where take is the returned Nat elements of the list
;; - where drop is the dropped Nat elements of the list
;; Interpretation: Returned and dropped elements of a list.

;; Examples:
(define TD-0 (make-td '() '()))
(define TD-1 (make-td (list 1) (list 2)))
(define TD-2 (make-td (list 1 2) (list 3)))
...
;; take-and-drop : [Listof X] Nat[>0] -> [Listof X]
;; Returns up the Nat elements of the list while dropping
;; Nat elements of the list.

(check-expect (take-and-drop '() 0) TD-0)
(check-expect (take-and-drop (list 1 2) 1) TD-1)
(check-expect (take-and-drop (list 1 2 3) 2) TD-2)

(define (take-and-drop lox nat) 
  (cond
    [(or (empty? lox) (zero? nat)) (make-td '() lox)]
    [(and (cons? lox) (positive? nat))
     (local [(define TD (take-and-drop (rest lox) (sub1 nat)))]
       (make-td
        (cons
         (first lox)
         (td-take TD))
